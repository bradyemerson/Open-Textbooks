...
define('DEBUG_REQUEST', false); //used to debug curl_requests.
...
function curl_request($options) { //single custom cURL request.
    ...
    if (DEBUG_REQUEST) {
        $options[CURLOPT_HEADER] = true; //used for debugging
    }
    $options[CURLINFO_HEADER_OUT] = true;
    $options[CURLOPT_VERBOSE] = true;
    $options[CURLOPT_RETURNTRANSFER] = true;
    $options[CURLOPT_COOKIEJAR] = CACHE_DIR . $user_token . '_cookies.txt';
    $options[CURLOPT_COOKIEFILE] = CACHE_DIR . $user_token . '_cookies.txt';

    //Randomize user agent if it's not set
    if (!isset($options[CURLOPT_USERAGENT])) { //default user agent.
        $options[CURLOPT_USERAGENT] = random_user_agent();
    }

    //set all options, both constant and user supplied.
    curl_setopt_array($ch, $options);

    if (DEBUG_REQUEST) {
        trigger_error("Calling " . print_r($options, true));
    }
    $response = curl_exec($ch);

    if (DEBUG_REQUEST || !trim($response) || curl_error($ch)) {
        $error = '';
        $info = curl_getinfo($ch);
        if ($info['request_header']) {
            $error .= $info['request_header'] . ' yielded ';
        }
        if (trim($response)) {
            $error .= 'response: '/* . htmlspecialchars($response) */;
        } else {
            $error .= 'no response';
        }
    ...
    if (curl_getinfo($ch, CURLINFO_EFFECTIVE_URL) != $options[CURLOPT_URL]) {
        trigger_error('Request to ' . $options[CURLOPT_URL] .
                ' was redirected to ' .
...
function random_user_agent() { //taken from http://www.danmorgan.net/programming/php-black-hat/random-useragent-in-curl-and-php/
    $browser_freq = array(
